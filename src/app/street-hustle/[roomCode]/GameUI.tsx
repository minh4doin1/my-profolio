...
export default function GameUI({
  ...
  isMyTurn,
  onRollDice,
  onEndBuildingTurn, // Nhận hàm mới
    ...
    <div className="w-full bg-gray-900 p-4 text-white flex justify-between items-center flex-shrink-0 border-t-2 border-blue-500 shadow-lg z-10">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mt-2 h-16 overflow-y-auto p-1 bg-black/20 rounded">
            {myBusinessTiles.length > 0 ? myBusinessTiles.map(tile => (
              <span key={tile.id} className="bg-purple-800 text-purple-200 px-2 py-1 rounded text-sm capitalize">
      ...
      <div className="w-80 flex-shrink-0 ml-6 flex flex-col items-center">
        ...
        <div className="w-full mt-2 grid grid-cols-2 gap-2">
          ...
          {/* NÚT XÂY DỰNG / KẾT THÚC LƯỢT XÂY */}
          <button
            onClick={onEndBuildingTurn}
            disabled={!isMyTurn || phase !== 'building'}
            className="col-span-2 px-4 py-3 font-bold text-white bg-orange-600 rounded-md hover:bg-orange-700 disabled:bg-gray-600 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all mt-2"
          >
            <Hammer />
